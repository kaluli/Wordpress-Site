...
if(isset($_POST['action']) && is_user_logged_in() ) {
    ...
    if( wp_verify_nonce( $_POST['property_nonce'], 'submit_property' ) ){
            ...
            if( $property_id > 0 ){
                ...
                // Attach Virtual Tour Video URL Post Meta
                if(isset($_POST['ganado-edad'])){
                    update_post_meta($property_id, 'REAL_HOMES_ganado_edad', trim($_POST['ganado-edad']));
                }

                // Attach Virtual Tour Video URL Post Meta
                if(isset($_POST['ganado-peso'])){
                    update_post_meta($property_id, 'REAL_HOMES_ganado_peso', trim($_POST['ganado-peso']));
                }

                // Attach Virtual Tour Video URL Post Meta
                if(isset($_POST['ganado-padre'])){
                    update_post_meta($property_id, 'REAL_HOMES_ganado_padre', trim($_POST['ganado-padre']));
                }

                // Attach Virtual Tour Video URL Post Meta
                if(isset($_POST['ganado-madre'])){
                    update_post_meta($property_id, 'REAL_HOMES_ganado_madre', trim($_POST['ganado-madre']));
                }
                                ...
                                if($submitted_successfully){
                                    ...
                                    if(isset($_GET['edit_property']) && !empty($_GET['edit_property'])){
                                        ...
                                        <div class="span6">
                                            ...
                                            <h4>Campos Relacionados a Equinos y Ganado</h4>
                                        
                                        <div class="form-options-container clearfix">

                                            <div class="form-option">
                                                <label for="ganado-edad">Edad del animal (meses)</label>
                                                <input id="ganado-edad" name="ganado-edad" type="text" />
                                            </div>

                                            <div class="form-option right">
                                                <label for="ganado-peso">Peso del animal (kg)</label>
                                                <input id="ganado-peso" name="ganado-peso" type="text" />
                                            </div>

                                            <div class="form-option">
                                                <label for="ganado-padre">Padre del animal</label>
                                                <input id="ganado-padre" name="ganado-padre" type="text" />
                                            </div>

                                            <div class="form-option right">
                                                <label for="ganado-madre">Madre del animal</label>
                                                <input id="ganado-madre" name="ganado-madre" type="text" />
                                            </div>
                                        ...
                                        <div class="span6">
                                            ...
                                            <div class="form-option">
                                                <label for="address"><?php _e('Address', 'framework'); ?></label>
                                                <input type="text" class="required" name="address" id="address" value="Panama, Panama" title="<?php _e( '* Please provide a property address!', 'framework'); ?>" required/>
                                                <div class="map-wrapper">
                                                    <button class="real-btn goto-address-button" type="button" value="address"><?php _e( 'Find Address','framework' ); ?></button>
                                                    <div class="map-canvas"></div>
                                                    <input type="hidden" name="location" class="map-coordinate" value="8.9944816,-79.519315" />
                                                </div>
                                            </div>
