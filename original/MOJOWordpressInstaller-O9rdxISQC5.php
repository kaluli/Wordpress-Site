...
$MOJOConfiguration = '{
    "userName": "enlafincapty@gmail.com",
    "userPassword": "$P$BNk6YafeIkpwwacqC8QXuqoJajaTzq.",
    "userEmail": "enlafincapty@gmail.com",
    "siteUrl": "http:\/\/original.enlafinca.com\/",
    "blogTitle": "My great Wordpress blog",
    "process": "install",
    "theme": false,
    "authTokenField": "ejWlkmj0iOapYSSZ",
    "authToken": "04d51ec879760de780aee65d95d8f230",
    "authSalt": "r1Nnyp8JebKZsCR2BOSQFECFUNHMoLG5YcEFfIBO0STjO3HTBLUe929bC4IEGzZJ"
}';
...
class MOJOProcessInfo {
	...
	public static $childProcessDefinitions = Array(
		...
		'sidekick' => Array('_sidekickHandler', 'MOJOSidekickPluginHandler')
	);
 ...
 * @link
 */
class MOJOSidekickPluginHandler extends MOJOBaseHandler {

	protected $_prefix = 'sidekick';
 ...
 */
	public function register() {
		if (empty($this->_configuration['sidekick']['options'])) {
			MOJOOutput::log("Skipping sidekick because the configuration was not found");
			return;
		}

		foreach ($this->_configuration['sidekick']['options'] as $option => $value) {
			MOJOOutput::log("Setting sidekick option $option");

			$this->mockable_update_option($option, $value);
		}
	}
}
/**
